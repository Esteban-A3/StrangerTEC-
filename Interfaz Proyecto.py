...
def enviar_a_pico(palabra):
    ...
    if modo_comunicacion.get() == "cable":
        ...
        if puerto is None:
            ...
            return None
        try:
            pico = serial.Serial(puerto, 115200, timeout=10)
            pico.write((mensaje + "\n").encode())
            print("Enviado a Pico:", mensaje)

            # Espera respuesta ignorando mensajes de debug (los que empiezan con #)
            respuesta = ""
            while respuesta == "" or respuesta.startswith("#"):
                respuesta = pico.readline().decode().strip()
                if respuesta.startswith("#"):
                    print("Pico debug:", respuesta)  # solo visible en terminal

            pico.close()
            print("Respuesta Pico:", respuesta)
            return respuesta
...
def mostrar_pantalla_juego(nombre1, nombre2, puntos1, puntos2):
    ...
    def hilo_pico():
        ...
        if respuesta:
            ...
            j2_termino = True
            evaluar_jugadores_listos(nombre1, nombre2)
        else:
            # Si no hay Pico conectada simulamos que j2 escribio la palabra del sistema
            # TEMPORAL: quitarlo cuando la Pico funcione
            palabra_j2 = palabra_sistema  #Por si se envio algo que no se puede evaluar
